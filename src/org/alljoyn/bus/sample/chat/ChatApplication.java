...
public class ChatApplication extends Application implements Observable {
    ...
    public void onCreate() {
        ...
        //set the channel to a random number
        Random r = new Random();
        hostRandomChannel = HOST_CHANNEL_PREFIX + r.nextInt();
        hostSetChannelName(hostRandomChannel);
        hostInitChannel();
        hostStartChannel();
	}
    ...
    public synchronized void addFoundChannel(String channel) {
    	...
    	// if we want a connection, we should try to connect to this new channel
    	if(mAppStatus == AppStatus.SEEKING_CONNECTION_INACTIVE) {
    		randomChat();
    	}
	...
	//private HashMap mChannelConnectionTime = new HashMap<String, Long>();
	/**
	...
	public synchronized void useJoinChannel() {
		/*Long lastConnectionTime = (Long)mChannelConnectionTime.get(mUseChannelName);
		if(lastConnectionTime != null) {
			if(SystemClock.uptimeMillis() - lastConnectionTime < 10000) {
				return;
			}
		}
		
		mChannelConnectionTime.put(mUseChannelName, SystemClock.uptimeMillis());*/
		clearHistory();
		notifyObservers(USE_CHANNEL_STATE_CHANGED_EVENT);
	...
	public synchronized void useLeaveChannel() {
		/*if(!mUseChannelName.equals(mHostChannelName)) {
			mChannelConnectionTime.put(mUseChannelName, SystemClock.uptimeMillis());
		}*/
		notifyObservers(USE_CHANNEL_STATE_CHANGED_EVENT);
	...
	public synchronized void setAppSessionAttributes(String name, int id) {
		mAppSessionName = name;
		mAppSessionId = id;
	}
	
	public synchronized int getAppSessionId() {
		return mAppSessionId;
	}
	
	public synchronized String getAppSessionName() {
		return mAppSessionName;
	}
	
	public synchronized void setAppDefaults() {
		setAppStatus(ChatApplication.AppStatus.IDLE);
		notifyObservers(SET_DEFAULTS_EVENT);
	}
	
	public synchronized void randomChat() {
		Log.i(TAG, "randomChat()");
		
		// the CONNECTION_ATTEMPTED_EVENT will cause the UseActivity to start the spinning progress bar
		notifyObservers(CONNECTION_ATTEMPTED_EVENT);
		
		String hostChan = hostGetChannelName();
		Log.i(TAG, "hostChan: " + hostChan);
		Integer hostNum = Integer.valueOf(hostChan.substring(HOST_CHANNEL_PREFIX.length()));
		Log.i(TAG, "hostNum: " + hostNum);
		List<String> channels = getFoundChannels();
		String chanStr;
		Integer chanNum;
		
		// randomly order the channels
		Random r = new Random();
		Collections.shuffle(channels, r);
		
		// find a channel with a greater number than ours and attempt to connect to it
		while(!channels.isEmpty() && mAppStatus == AppStatus.SEEKING_CONNECTION_ACTIVE) {
			chanStr = channels.get(0);
			Log.i(TAG, "chanStr: " + chanStr);
			chanNum = Integer.valueOf(chanStr.substring("org.alljoyn.bus.samples.chat.".length() + HOST_CHANNEL_PREFIX.length()));
			Log.i(TAG, "chanNum: " + chanNum);
			channels.remove(0);
			
			if(chanNum > hostNum) {
				Log.i(TAG, "Attempting to connect");
				useSetChannelName(chanStr.substring("org.alljoyn.bus.samples.chat.".length()));
				useJoinChannel();
				return;
			}
		}
		
		// No valid channels were found. This flag will cause us to check again when new channels are found.
		if(mAppStatus == AppStatus.SEEKING_CONNECTION_ACTIVE) {
			setAppStatus(AppStatus.SEEKING_CONNECTION_INACTIVE);
		}
	}
	
	// the AlljoynService should call this in order to find out if we are seeking a connection or not
	// when deciding whether to accept or reject a connection request
	public synchronized AppStatus getAppStatus() {
		return mAppStatus;
	}
	
	public synchronized void setAppStatus(AppStatus as) {
		Log.i(TAG, "setAppStatus(" + as + ")");
		mAppStatus = as;
	}
	...
	// this should be called by AlljoynService when a connection attempt is successful
	public synchronized void connectionAccepted() {
		mAppStatus = AppStatus.CONNECTED;
		
		// this will stop the spinning progress bar
		notifyObservers(CONNECTION_ESTABLISHED_EVENT);
	}
	
	// this should be called by AlljoynService when a connection attempt is rejected by the host
	public synchronized void connectionRejected() {
		//try it again since it didn't work the first time
		randomChat();
	}
	 ...
	 */
	public synchronized void addHistoryItem(String nickname, String message) {
