...
public class AllJoynService extends Service implements Observer {
    ...
    private class SessionGuardian extends Semaphore{
    	...
    	public SessionGuardian() {
    		...
    		// we set the timeAcquired to the max value so that timeSinceLastAcquired
    		// will return a large negative number, so that we wont satisfy our timeout 
    		// condition at the start.
    		timeAcquired=Long.MAX_VALUE;
    	}
    	
    	public boolean tryAcquire() {
    		boolean acquired = super.tryAcquire();
    		Log.i(TAG, "tryAcquire()");
    		long timeSinceLastAcquired = timeSinceLastAcquired();
    		if(acquired || timeSinceLastAcquired > TIMEOUT_MS) {
    			Log.i(TAG, "tryAcquire(), success acquired=" + acquired + ", timeSinceLastAcquired=" + timeSinceLastAcquired );
    			timeAcquired = SystemClock.uptimeMillis();
    			return true;
    		}
    		else {
    			Log.i(TAG, "tryAcquire(), failed acquired=" + acquired + ", timeSinceLastAcquired=" + timeSinceLastAcquired );
    			return false;
    		}
    		
    	}
    	
    	// we need to handle server permits differently because it is possible for the TIMEOUT
    	// to expire, causing multiple permits to have been issued. We must check against some
    	// id to see if the permit is still valid. We use the id of the joiner.
    	/*public boolean tryAcquireServerPermit(String joiner) {
    		
    	}*/
    	
    	public void release() {
    		Log.i(TAG, "release()");
    		// in case we accidentally released too many times, we should reset
    		// the number of permits to zero before releasing again. This might
    		// happen if our connection timeout expired, but the connection still occurred.
    		drainPermits();
    		timeAcquired=Long.MAX_VALUE;
    		super.release();
    	}
    	
    	private long timeSinceLastAcquired() {
    		return SystemClock.uptimeMillis() - timeAcquired;
    	}
    }
        ...
        Status status = mBus.bindSessionPort(contactPort, sessionOpts, new SessionPortListener() {
        	...
        	public boolean acceptSessionJoiner(short sessionPort, String joiner, SessionOpts sessionOpts) {
        		...
        		if (sessionPort == CONTACT_PORT) {
        			...
        			if(mChatApplication.getAppStatus() == ChatApplication.AppStatus.SEEKING_CONNECTION_ACTIVE || 
        			   mChatApplication.getAppStatus() == ChatApplication.AppStatus.SEEKING_CONNECTION_INACTIVE) {
        				Boolean sessionPermit = mSessionGuardian.tryAcquire();
                        Log.i(TAG, "acceptSessionJoiner(), sessionPermit=" + sessionPermit);
            ...
            public void sessionJoined(short sessionPort, int id, String joiner) {
                ...
                mBus.setSessionListener(id, new SessionListener() {
                    ...
                    public void sessionLost(int sessionId, int reason) {
                        ...
                        mSessionGuardian.release();
                     	mUseChannelState = UseChannelState.IDLE;
                      	mChatApplication.useSetChannelState(mUseChannelState);
                      	mChatApplication.setAppDefaults();
                      	mChatApplication.addHistoryItem("", "*** The User Has Disconnected ***");
                ...
                mChatApplication.setAppSessionAttributes(joiner, id);
                
                // we now need to join our own channel
                String hostChannelName = mChatApplication.hostGetChannelName();
    			mChatApplication.useSetChannelName(hostChannelName);
    			mChatApplication.useJoinChannel();
    			mChatApplication.connectionAccepted();
    			mChatApplication.addHistoryItem("", "*** Connected to User ***");
            }             
        });
    ...
    private void doJoinSession() {
        ...
        Log.i(TAG, "doJoinSession() joining session, wellKnownName=" + wellKnownName + ", contactPort=" + contactPort + ", sesionOpts=" + sessionOpts.toString());
        
        Boolean permit = mSessionGuardian.tryAcquire();
        // add the channel to the list
        Long lastConnectionTime = (Long)mChannelConnectionTime.get(mChatApplication.useGetChannelName());
		if(lastConnectionTime == null || SystemClock.uptimeMillis() - lastConnectionTime > 10000) {
			mChannelConnectionTime.put(mChatApplication.useGetChannelName(), SystemClock.uptimeMillis());
		}
        
	    if(permit && (lastConnectionTime == null || SystemClock.uptimeMillis() - lastConnectionTime > 10000)) {
	        Status status = mBus.joinSession(wellKnownName, contactPort, sessionId, sessionOpts, new SessionListener() {
	        	...
	        	@Override
	            public void sessionMemberAdded (int sessionId, String uniqueName) {
	            	Log.i(TAG, "sessionMemberAdded(sessionId=" + sessionId + ", uniqueName=" + uniqueName);
	            }
	            
	        	@Override
	            public void sessionMemberRemoved (int sessionId, String uniqueName) {
	            	Log.i(TAG, "sessionMemberRemoved(sessionId=" + sessionId + ", uniqueName=" + uniqueName);
	            	
	            	// if someone leaves the session, the other should leave too
	            	mChatApplication.useLeaveChannel();
	            }
	        });
	        Log.i(TAG, "doJoinSession() finished joining session");
	        
	        if (status == Status.OK) {
	        	Log.i(TAG, "doJoinSession(): success");
	            Log.i(TAG, "doJoinSession(): success use sessionId is " + sessionId.value);
	        	mUseSessionId = sessionId.value;
	        	mChatApplication.connectionAccepted();
	        	mChatApplication.addHistoryItem("", "*** Connected to User ***");
	        } else {
	        	Log.i(TAG, "doJoinSession(): failure, Status=" + status);
	        	Log.i(TAG, "doJoinSession(): failure use sessionId is " + sessionId.value);
	        	mSessionGuardian.release();
	        	SystemClock.sleep(1000);
	        	mChatApplication.connectionRejected();
	        	
	        	//we no longer want to have an error since we are supposed to reject some connections. Such as when we are already connected to someone.
    ...
    private void doLeaveSession() {
        ...
        if (mJoinedToSelf == false) {
        	...
        	mBus.removeSessionMember(mChatApplication.getAppSessionId(), mChatApplication.getAppSessionName());
        }
